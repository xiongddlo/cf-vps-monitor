...
test('AUD-15 fifty nodes and two Ping tasks exceed the independent free DO write allowance', async () => {
  ...
  assert.equal(writes?.websocket, 108_000, '36,000 history markers plus 72,000 per-task states must be counted');
  assert.equal(writes.http, 144_000, 'HTTP reporting additionally persists 36,000 live states');
...
test('AUD-15 task count, disabled history, and active reporting affect the right resource dimensions', async () => {
  for (const [pingCount, expected] of [[0, 36_000], [1, 72_000], [2, 108_000]]) {
  ...
  assert.equal(disabled.resource_estimates.find(row => row.key === 'durable_object_rows_written').websocket, 0);
