...
assert.deepEqual(normalizeSettingValue('update_mode', 'actions'), { ok: true, value: 'actions' });
assert.deepEqual(normalizeSettingValue('update_mode', 'fork'), { ok: true, value: 'fork' });
assert.equal(normalizeSettingValue('update_mode', 'branch').ok, false);
assert.deepEqual(
