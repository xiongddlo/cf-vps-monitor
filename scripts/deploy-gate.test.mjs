...
function runDeployment({ verificationStatus = 0, args = [], preparationFails = false } = {}) {
  ...
  const fakeProcess = { platform: process.platform, execPath: process.execPath, argv: ['node', fileURLToPath(deploymentUrl), ...args],
    env: { SUPABASE_URL: 'https://synthetic.supabase.co', JWT_SECRET: 'synthetic-jwt-secret', SUPABASE_SECRET_KEY: 'sb_secret_synthetic' },
    exit: status => { throw new Exit(status); } };
  const spawn = (command, argv) => {
    calls.push({ command, args: argv });
    if (/npm(?:\.cmd)?$/.test(command)) return { status: verificationStatus, stdout: '', stderr: '' };
