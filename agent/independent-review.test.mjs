...
. "$review_installer"
`);
  const args = [shellPath(wrapper), shellPath(join(repo, 'agent', scriptName)),
    '--dry-run', '--instance-id', 'review-fixture', '--service-name', serviceName,
  ...
  const result = spawnSync(scriptName === 'install.sh' ? 'sh' : bashExecutable(), args, {
    ...
    env: { ...process.env, REVIEW_PLATFORM: platform,
